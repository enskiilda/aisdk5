        ...
        >
          <div className="flex flex-col w-full">
            {message.parts?.map((part, i) => {
              ...
              // Handle tool parts (SDK 5 format: tool-computer, tool-bash, etc.)
              if (part.type.startsWith("tool-")) {
                const toolPart = part as unknown as ToolPartBase & { type: string };
                const toolName = part.type.replace("tool-", "");
                const { toolCallId, state, input, output } = toolPart;

                // Check if this is a pending/in-progress state
                const isPending = state === "input-streaming" || state === "input-available";
                const isComplete = state === "output-available";

                if (toolName === "computer") {
                  const args = (input || {}) as ComputerInput;
                  const {
                    action = "",
                    coordinate,
                    text,
                  ...
                  const result = output as ImageOutput | TextOutput | string | undefined;
                  const isAborted = result === ABORTED;
                  const isImageResult = typeof result === "object" && result?.type === "image";
                  ...
                  return (
                    ...
                    >
                      ...
                      {isComplete && isImageResult ? (
                        <div className="p-2">
                          {/* eslint-disable-next-line @next/next/no-img-element */}
                          <img
                            src={`data:image/png;base64,${(result as ImageOutput).data}`}
                            alt="Generated Image"
                            className="w-full aspect-[1024/768] rounded-sm"
                          />
                        </div>
                      ) : !isComplete && action === "screenshot" ? (
